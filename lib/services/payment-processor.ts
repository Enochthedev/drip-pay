...
import { processSubscriptionPayment, getTokenBalance } from '../blockchain/contracts/subscription-service'
 ...
 * Processes due subscription payments and handles failures
 */
 ...
 */
export async function processAllDuePayments(): Promise<ProcessingResult> {
  ...
  try {
    ...
    for (const subscription of dueSubscriptions) {
      try {
        await processSinglePayment(subscription)
        result.processed++
      } catch (error) {
 ...
 */
async function processSinglePayment(subscription: any): Promise<void> {
...
async function handlePaymentFailure(subscription: any, error: string): Promise<void> {
  ...
  if (failedCount >= MAX_FAILED_PAYMENTS) {
    ...
    console.log(`⚠️ Payment failed for subscription ${subscription.id} (Attempt ${failedCount}/${MAX_FAILED_PAYMENTS})`)
  }
}
...
/**
 * Send payment reminders (24 hours before due)
 */
export async function sendPaymentReminders(): Promise<void> {
  ...
  for (const subscription of upcomingSubscriptions) {
    if (subscription.subscriber.email) {
      const { sendPaymentReminder } = await import('../notifications/email')
      const chainConfig = getChainConfig(subscription.chainId)
      const token = chainConfig?.supportedTokens.find(t => t.address === subscription.plan.tokenAddress)

      if (token) {
        await sendPaymentReminder(
          subscription.subscriber.email,
          subscription.plan.name,
          formatUnits(BigInt(subscription.plan.price), token.decimals),
          token.symbol,
